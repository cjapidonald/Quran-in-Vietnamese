...
struct ReaderDashboardView: View {
    ...
    var body: some View {
        ...
        .onChange(of: readerStore.lastLanguageEnforcementID) { _, newValue in
            ...
            showToast(message: "Luôn giữ ít nhất một ngôn ngữ hiển thị")
        }
    ...
    private var surahSearchSection: some View {
        VStack(alignment: .leading, spacing: DesignTokens.Spacing.md) {
            ...
            if shouldShowSearchResults {
                ...
                Text("Không có chương nào khớp với \"\(trimmedSurahSearchText)\"")
                    .font(.caption)
    ...
    private func ayahCard(for ayah: AyahPlaceholder) -> some View {
        ...
        .contextMenu {
            Button {
                ...
                Label("Sao chép câu", systemImage: "doc.on.doc")
    ...
    private var surahHeader: some View {
        VStack(alignment: .leading, spacing: DesignTokens.Spacing.xs) {
            Text(selectedSurah.name)
                ...
                .foregroundStyle(primaryText)
            Text("Chế độ: \(readerStore.isFlowMode ? "Liên tục" : "Theo câu")")
                .font(.footnote.weight(.medium))
                .foregroundStyle(secondaryText.opacity(0.8))
        }
        .frame(maxWidth: .infinity, alignment: .leading)
